"""제출 sub_014: sub_013 + g3 sister shift150 병행 (raw150+shift150, 홀드아웃 0.6357 최적 구성).
...
def main() -> None:
    ...
    for tgt in TARGET_COLS:
        ...
        if tgt == "kpx_group_3":
            ...
            om_raw = interp_atoms(sq, n=K)
            med_g = np.median(gbm_atoms, axis=1)
            om_shift = np.clip(om_raw + (med_g - np.median(om_raw, axis=1))[:, None], 0, cap)
            parts += [om_raw, om_shift]
            print(f"g3: 혼합 +{n8+n6}, OM sister raw+shift +{2*K} 원자", flush=True)
    ...
    path = PROJECT / "submissions" / "sub_014_g3_omsister_rs.csv"
