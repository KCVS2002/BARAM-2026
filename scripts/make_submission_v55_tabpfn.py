# -*- coding: utf-8 -*-
"""sub_055: TabPFN sister LB 프로브 (#133 후속) — sub_053 + g1/g2 TabPFN 75원자.
...
def main() -> None:
    ...
    for tgt_i, tgt in enumerate(TARGET_COLS):
        ...
        if tgt == "kpx_group_3":
            ...
            # ── TabPFN sister (CV 프로토콜 동일) ──
            trm = df[tgt].notna()
            tr = df[trm]
            wv = w_q3[tgt].to_numpy()[trm.to_numpy()]
            rng_t = np.random.default_rng(42)
            p = wv / wv.sum()
            idx = rng_t.choice(len(tr), size=min(CTX, len(tr)), replace=False, p=p)
            ...
            print(f"{tgt}: TabPFN 추론 완료 ({time.time()-t0:.0f}s)", flush=True)
    ...
    path = PROJECT / "submissions" / "sub_055_tabpfn.csv"
