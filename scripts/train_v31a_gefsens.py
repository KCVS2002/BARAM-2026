"""v31a: GEFS 앙상블 평균(geavg 전 기간) 피처 — feat5 위에 +2 (컨센서스 강화).
...
def main() -> None:
    ...
    g = g[g.member == "avg"]
    g["forecast_kst_dtm"] = (pd.to_datetime(g["run"].astype(str), format="%Y%m%d%H")
                             + pd.Timedelta(hours=9) + pd.to_timedelta(g.fxx, unit="h"))
    gp = g.pivot_table(index="forecast_kst_dtm", columns="var", values="value", aggfunc="mean")
    ens = np.sqrt(gp["u10"] ** 2 + gp["v10"] ** 2).rename("gefs_ens_ws10").reset_index()
    full = pd.date_range(ens.forecast_kst_dtm.min(), ens.forecast_kst_dtm.max(), freq="h")
    ens = ens.set_index("forecast_kst_dtm").reindex(full).interpolate(limit=2)
    ens.index.name = "forecast_kst_dtm"
    df = df.merge(ens.reset_index(), on="forecast_kst_dtm", how="left")
    df["gefs_ens_det_diff"] = df["gefs_ens_ws10"] - df["gfs_ws10"]
    base_cols = [c for c in feat.columns if c != "forecast_kst_dtm"]
    ifs_cols = ["ifs_ws10", "ifs_ws925", "ifs_ws850", "cons3_mean", "cons3_std"]
    dir_cols = ifs_cols + ["gefs_ens_ws10", "gefs_ens_det_diff"]
    ...
    for m0 in range(1, 12, 2):
        ...
        for tgt in TARGET_COLS:
            ...
            for v, (qp_use, anen_use) in {
                "feat5": (qp_f, anen_b), "ens7": (qp_d, anen_b),
            }.items():
