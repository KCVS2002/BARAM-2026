"""제출 sub_027: sub_026에서 조건 신호를 거리(d̄)→수준(med/cap)으로 교체 (실험 #82 v50).

v50 정식 판정: lvl(+0.0041, 5/6, FICR +0.0085) > dist(+0.0029), 가산은 과기울임으로 열세.
고출력 시간일수록 실현 분포(AnEn)의 밴드 적중력이 크다는 메커니즘 (#79 g3 고풍속
진단과 정합). 본체 재배분과 g3 sister 조건부 모두 같은 신호(수준 3분위)로 통일.
...
def main() -> None:
    ...
    for tgt in TARGET_COLS:
        ...
        # 수준 조건부 재배분 (#82 v50 lvl): 예측 수준(med/cap) 3분위 —
        # 고수준 anen175 / 중간 150 / 저수준 125 (고출력 시간 = 실현 분포의 밴드 적중력)
        med_lvl = med / cap
        terc = np.searchsorted(np.quantile(med_lvl, [1 / 3, 2 / 3]), med_lvl)
        ...
        print(f"{tgt}: 수준 3분위 분포 {n_terc.tolist()}", flush=True)
    ...
    path = PROJECT / "submissions" / "sub_027_lvl.csv"
