"""제출 sub_023: sub_021 + 원자 폭 +7% (중앙값 기준 확산) — #61 통일 이론의 미실증 절반 프로브.

근거: 좁히기는 LB 3연속 실패 실증이 있으나 넓히기는 CV에서 져서 제출된 적이 없음.
이론("2024 검증의 과분산 = 2025 전이 오차의 마진")이 맞다면 2025 최적 폭은 현 폭보다
넓은 쪽일 수 있다. 이론 예측은 소폭 손실 — 이득이면 새 축 개방, 손실이면 이론 확증.
...
def main() -> None:
    ...
    for tgt in TARGET_COLS:
        ...
        # 폭 +7%: 시간별 중앙값 고정, 산포만 확대 (위치 불변 — 순수 폭 축 프로브)
        med_a = np.median(atoms, axis=1)
        atoms = np.sort(np.clip(med_a[:, None] + (atoms - med_a[:, None]) * 1.07, 0, cap), axis=1)
        a = df.loc[trm, tgt]
    ...
    path = PROJECT / "submissions" / "sub_023_widen07.csv"
