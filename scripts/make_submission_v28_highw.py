"""제출 sub_028: sub_026(공식) + 고출력 가중 학습 w2 (실험 #83 v51: CV +0.0020,
FICR +0.0043 / 1-NMAE -0.0003). main GBM 학습 가중에 ×(1+2·cf) — 오차 예산을
FICR의 에너지 가중(연도 불변 산식 구조)에 정렬. sister 가중 불변 (검증 범위 격리).
...
def main() -> None:
    ...
    for tgt in TARGET_COLS:
        ...
        weights_main[tgt] = weights_main[tgt].to_numpy() * (1 + 2 * np.clip(cf, 0, 1))
    om = load_om()
    ...
    path = PROJECT / "submissions" / "sub_028_highw.csv"
