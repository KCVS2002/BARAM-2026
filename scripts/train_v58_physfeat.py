"""#91 v58: 이론 조사(domain_theory.md) 티어1 물리 피처 절제 실험.

기존 데이터만 사용, 물리 메커니즘 명확 (CV 신뢰 가능 클래스 — 구조·물리 변경).
- veff : 밀도보정 유효풍속 v·(ρ/1.225)^(1/3) (IEC 피치형 보정). air_density·wpd는
         기존 피처에 이미 있어 잔여분(무릎 좌우 이동의 직접 인코딩)만 검증.
- ridge: 능선 상대 풍향 — 말굽 배치 두 다리 주향(약 N20°E/N140°E, OSM 실측)에 대한
         수직 성분 U⊥ = ws·|sin(wd−az)| (능선 가속·웨이크 공통 인자) + Froude 수
         Fr = U⊥850/(N·h) (N: IFS t925/dt → 온위차, h=1000m; 파동 증폭/차단 레짐).
- hist : 강풍 이력 — 일 블록 내 비대칭(과거만) 롤링 최대 풍속 3h/6h (ldaps_ws50max,
         gfs_ws100, gfs gust). 컷아웃(22~25m/s) 히스테리시스·경로의존의 프록시.
변형: base(현행 q3 공식) / veff / ridge / hist / all. 하네스 v56 동일 (6-fold q3).
판정 눈금: 6-fold 평균 ±0.0016 = 2σ. 풀링 채점 병기.
"""
...
def main() -> None:
    ...
    VARIANTS = {
        "base": cols0,
        "veff": cols0 + grp_cols["veff"],
        "ridge": cols0 + grp_cols["ridge"],
        "hist": cols0 + grp_cols["hist"],
        "all": cols0 + grp_cols["veff"] + grp_cols["ridge"] + grp_cols["hist"],
    }
    ...
    print("\n=== v58 물리 피처 절제 — fold평균 | 풀링 (총/1-NMAE/FICR) ===")
    for v in variants:
