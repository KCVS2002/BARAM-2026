"""제출 sub_032: sub_029에서 무효시간(<10%C) 완만 다운웨이트 ×0.7 추가 (#88, v56 sv07).
main GBM 가중 = ×(1+3·cf²) × (cf<0.1 ? 0.7 : 1) — 산식 구조 정렬의 잔여 절반(유효시간).
CV: sv07 +0.0031 (노이즈 바 ±0.0016의 2배), NMAE +0.0005 / FICR +0.0056 파레토형.
v43/sub_024(급격 ×0.2 단독, LB -0.0020)와 구별: 완만 강도 + q3 결합. sister는 불변.
...
def main() -> None:
    ...
    # 고출력 가중 q3 + 무효시간 ×0.7 (#88 sv07): main GBM 전용 — sister는 불변
    weights_main = weights.copy()
    for tgt in TARGET_COLS:
        cf = (lab[tgt] / CAPACITY_KWH[tgt]).fillna(0).to_numpy()
        w_sv = (1 + 3 * np.clip(cf, 0, 1) ** 2) * np.where(cf < 0.1, 0.7, 1.0)
        weights_main[tgt] = weights_main[tgt].to_numpy() * w_sv
    om = load_om()
    ...
    path = PROJECT / "submissions" / "sub_032_sv07.csv"
