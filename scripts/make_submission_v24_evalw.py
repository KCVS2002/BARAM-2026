"""제출 sub_024: sub_021 + 유효시간 지향 가중 λ=0.2 (실험 #74 v43: CV +0.0013,
1-NMAE +0.0019 — 단 fold 3/6 고분산이라 LB 프로브). 무효시간(<10%cap) 학습 가중 ×0.2,
main GBM에만 적용 (sister 불변 — v43 검증 범위와 동일).
...
def main() -> None:
    ...
    # 유효시간 지향 가중 (#74): 무효시간(<10%cap) ×0.2 — main GBM 전용
    weights_main = weights.copy()
    for tgt in TARGET_COLS:
        invalid = (lab[tgt] < 0.10 * CAPACITY_KWH[tgt]).to_numpy()
        weights_main[tgt] = weights_main[tgt].to_numpy() * np.where(invalid, 0.2, 1.0)
    om = load_om()
    ...
    path = PROJECT / "submissions" / "sub_024_evalw02.csv"
