"""v49: 바람 벡터 성분 재표현 — GBM이 못 배우는 회전 상호작용의 명시화 (A 카드).
...
def main() -> None:
    ...
    # 벡터 성분 재구성 (기상학 wd → u=−ws·dsin, v=−ws·dcos), 대각 성분 = 45° 회전
    for src in ("ldaps_ws50max", "gfs_ws100"):
        u = -df[src] * df[f"{src}_dsin"]
        v = -df[src] * df[f"{src}_dcos"]
        df[f"{src}_u"] = u
        df[f"{src}_v"] = v
        df[f"{src}_u45"] = (u + v) / np.sqrt(2)
        df[f"{src}_u135"] = (u - v) / np.sqrt(2)
    ...
    uv = [f"{s_}_{c}" for s_ in ("ldaps_ws50max", "gfs_ws100") for c in ("u", "v")]
    diag = [f"{s_}_{c}" for s_ in ("ldaps_ws50max", "gfs_ws100") for c in ("u45", "u135")]
    adds = {"base": [], "uv": uv, "uv8": uv + diag}
    new_cols = adds["uv8"]
