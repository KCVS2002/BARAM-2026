"""v55: 램프 게이트 3단 선택 (#87) — 고수준이라도 램프 중이면 super 대신 q3.

fold 해부(#86): sel3는 안정 기간(01/09/11 +31~95)에 이기고 램프 계절(03/05)에 짐 —
램프 중엔 수준 신호가 불안정해 선택이 오배정. 게이트: 국지 램프(전후 1h 예측 변화,
예측 집합 3분위) 상위면 super 투입 보류.
...
def main() -> None:
    ...
    variants = ["q3", "sel3", "sel3_rg"]
    res = {v: {} for v in variants}
    dec = {v: {} for v in variants}
    band_prof = {m: {b: [] for b in range(4)} for m in ("q3", "sel3_rg")}  # 진단
    for m0 in range(1, 12, 2):
        ...
        for tgt in TARGET_COLS:
            ...
            day = (sub["forecast_kst_dtm"] - pd.Timedelta(hours=1)).dt.date.to_numpy()
            ramp = np.zeros(len(med_sig))
            for i in range(len(med_sig)):
                lo = i - 1 if i > 0 and day[i - 1] == day[i] else i
                hi = i + 1 if i < len(med_sig) - 1 and day[i + 1] == day[i] else i
            ...
            def qp_of(v):
                ...
                else:  # sel3_rg: 고수준이라도 램프 상위 3분위면 q3
                    hi_ok = (terc == 2) & ~ramp_hi
                    hi_rp = (terc == 2) & ramp_hi
    ...
    for b in range(4):
        ...
        q = np.mean(band_prof["sel3_rg"][b])
        print(f"  {labels[b]}: q3 {p:.4f} / sel3_rg {q:.4f} ({(q-p)/p*+100:+.1f}%)")
