"""제출 sub_034: sub_029 + roll_c — 관측 기반 롤링 NWP 편향 보정 (#107 v70, 롤링 Phase 1).
**2025 관측이 파이프라인에 들어가는 최초 제출.** 각 대상일 D의 컷오프(D-1 13:00) 이전
관측(대관령·백운산·태백 합성 지수)으로 주 풍속 3피처(ldaps_ws50max·gfs_ws100·ifs_ws925)를
×exp(clip(anom7, ±0.25)) 보정. 전 연도 대칭 프로토콜 (학습도 동일 보정).
CV 동률(+0.0001/풀링 -0.0010) — 프로브 목적: CV(2024)가 원리적으로 측정 불가한
'2025 단지 수준 드리프트'의 실측. 누수 안전: AWS 관측은 실시간 공개, 컷오프 이전만 사용
(BARAM.md 실측 데이터 사용 기준 명시 허용).
...
from scripts.train_v70_rollbias import WIND_MAIN, build_rolling_bias
...
def main() -> None:
    ...
    # roll_c (#107): 롤링 관측 편향 보정 — 주 풍속 3피처를 보정 사본으로 교체 (main GBM 전용)
    rb = build_rolling_bias()
    for d_ in (df, te):
        d_["day"] = (d_["forecast_kst_dtm"] - pd.Timedelta(hours=1)).dt.normalize()
    df = df.merge(rb, on="day", how="left").drop(columns=["day"])
    te = te.merge(rb, on="day", how="left").drop(columns=["day"])
    for d_ in (df, te):
        fac = np.exp(np.clip(d_["anom7"].fillna(0), -0.25, 0.25))
        for c in WIND_MAIN:
            d_[f"{c}_corr"] = d_[c] * fac
    print(f"roll_c: train anom7 커버 {df['anom7'].notna().mean()*100:.1f}% / "
          f"test {te['anom7'].notna().mean()*100:.1f}% | test fac 범위 "
          f"{np.exp(np.clip(te['anom7'].fillna(0), -.25, .25)).min():.3f}~"
          f"{np.exp(np.clip(te['anom7'].fillna(0), -.25, .25)).max():.3f}", flush=True)
    ...
    # ── base GBM (전 기간 공유 학습) — 주 풍속 3피처는 보정 사본으로 교체 ──
    main_cols = [f"{c}_corr" if c in WIND_MAIN else c for c in feature_cols + ifs_cols]
    stack_X, stack_y, stack_w = [], [], []
    for tgt in TARGET_COLS:
        cap = CAPACITY_KWH[tgt]
        trm = df[tgt].notna()
        Xg = df.loc[trm, main_cols].copy()
    ...
    shared_cols = main_cols + ["g_rated", "g_rotor", "g_id"]
    ...
    path = PROJECT / "submissions" / "sub_034_rollc.csv"
