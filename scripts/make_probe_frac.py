"""배깅 결정층 프로브 4종 (#65 frac/B 축 LB 실측): sub_021과 동일 학습·원자에서
결정층만 변형 — frac 0.4 / 0.8 / 0.5, B=30. 파일은 완성되는 즉시 순서대로 저장.
...
def main() -> None:
    ...
    for tgt in TARGET_COLS:
        ...
        atoms_all[tgt] = np.sort(np.concatenate(parts, axis=1), axis=1)
        a = df.loc[trm, tgt]
        abar_all[tgt] = a[a >= cap * 0.10].mean()
        print(f"{tgt}: 원자 준비 완료 ({time.time()-t0:.0f}s)", flush=True)

    variants = [
        ("sub_022_frac04.csv", dict(B=15, frac=0.4)),
        ("sub_023_frac08.csv", dict(B=15, frac=0.8)),
        ("sub_024_frac05.csv", dict(B=15, frac=0.5)),
        ("sub_025_B30.csv", dict(B=30, frac=0.6)),
    ]
    for fname, kw in variants:
        out = sub[["forecast_id", "forecast_kst_dtm"]].copy()
        for tgt in TARGET_COLS:
            out[tgt] = optimize_bagged(atoms_all[tgt], CAPACITY_KWH[tgt], abar_all[tgt], **kw)
        out["forecast_kst_dtm"] = out["forecast_kst_dtm"].dt.strftime("%Y-%m-%d %H:%M:%S")
        path = PROJECT / "submissions" / fname
        ...
        print(f"saved {fname} {kw} ({time.time()-t0:.0f}s)", flush=True)
