"""v30: 다중 K AnEn (K=50/150/300 혼합) — 다중 스케일 밀도 추정, 상수 무의존 구조.

가설: 단일 K=150은 밀도 추정의 대역폭(bandwidth) 고정과 같다. 작은 K(국소 선명)와
큰 K(강건)를 섞으면 커널 대역폭 혼합처럼 조건부 밀도의 표현력이 오른다.
- multi: K50 원자 50 + K150→50 + K300→50 = 150개 (기존과 동수, 재정렬 동일)
- 기준: sub_009 구성 CV 0.6470 (base 재계산 포함)
"""
...
def sub50(a_sorted_row_wise: np.ndarray) -> np.ndarray:
    n = a_sorted_row_wise.shape[1]
    idx = np.linspace(0, n - 1, 50).astype(int)
    return a_sorted_row_wise[:, idx]
...
def main() -> None:
    ...
    for m0 in range(1, 12, 2):
        ...
        for tgt in TARGET_COLS:
            ...
            knn = NearestNeighbors(n_neighbors=300).fit(((tr_ok[ANEN_FEATS] - mu_a) / sd_a * FEAT_W).to_numpy())
            _, aidx = knn.kneighbors(((sub[ANEN_FEATS] - mu_a) / sd_a * FEAT_W).to_numpy())
            lib = (tr_ok[tgt] / cap).to_numpy() * cap
            an50 = np.sort(lib[aidx[:, :50]], axis=1)
            an150 = np.sort(lib[aidx[:, :150]], axis=1)
            an300 = np.sort(lib[aidx], axis=1)
            ...
            for v in res:
                if v == "base":
                    ...
                    anen = np.concatenate([an50, sub50(an150), sub50(an300)], axis=1)
                    anen = np.sort(anen, axis=1)
        ...
        print(f"fold {fold}: base={res['base'][fold]:.4f} multi={res['multi'][fold]:.4f} "
