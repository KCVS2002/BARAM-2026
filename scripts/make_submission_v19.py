"""제출 sub_019: sub_017 + 원자 결합을 half(Vincent 150+혼합 150)로 (실험 #61: CV +0.0039, FICR +0.008).
...
def main() -> None:
    ...
    for tgt in TARGET_COLS:
        ...
        anen_atoms = np.sort(np.clip(anen_atoms + shift[:, None], 0, cap), axis=1)
        # half 결합 (#61): Vincent(분위별 평균, 과분산 제거) + 혼합(다양성 헤지) 50:50
        vin = np.sort((gbm_atoms + anen_atoms) / 2, axis=1)
        mix = np.sort(np.concatenate([gbm_atoms[:, ::2], anen_atoms[:, ::2]], axis=1), axis=1)
        parts = [vin, mix]
    ...
    path = PROJECT / "submissions" / "sub_019_vincent_half.csv"
