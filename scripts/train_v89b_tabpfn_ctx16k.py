"""#137 v89: TabPFN 컨텍스트 16k 게이트 — 스케일링 끝점 (사실상 전체 풀) 검사.

v88: 6k→12k에서 품질↑ fold와 점수↑가 정합 (cmix +0.0014, 바 경계선).
16k는 CV per-그룹 풀(1.7만~2.1만행)을 초과 → 실질 '전체 풀 컨텍스트'.
주의: TabPFN v2 사전학습은 ≤10k행 — 한계 초과 외삽의 열화 여부도 이 게이트가 판별.

대상: fold 01(12k 최대 개선)·09·11(2025 인접) × g1/g2.
변형: base(tpf75 6k) / c16(16k 75 교체) / cmix16(6k 75 + 16k 50).
...
def main() -> None:
    ...
    for fold in FOLDS:
        ...
        for tgt in G12:
            ...
            q24 = np.load(CACHE / f"{fold}_{tgt}_tpfqp_c16k.npz")["qp"]
            pbg = pinball(actual, qp_gbm, QS)
            print(f"  {fold} {tgt[-1]}: pinball비 6k {pinball(actual,q6,QS)/pbg:.3f} / "
                  f"12k {pinball(actual,q12,QS)/pbg:.3f} / 16k {pinball(actual,q24,QS)/pbg:.3f} | "
                  f"GBM상관 16k {np.corrcoef(q24[:,9]-actual, qp_gbm[:,9]-actual)[0,1]:.3f}", flush=True)
    ...
    print("\n=== v89 16k 게이트 (g1/g2, fold 01/09/11) ===")
    for v in VARIANTS:
