"""v32c: 가치지향 모델 길들이기 배터리 — 정칙화·보상 강도별 4변형 용량-반응.
...
TAME = dict(learning_rate=0.01, num_leaves=15, min_child_samples=200,
            colsample_bytree=0.8, subsample=0.8, subsample_freq=1, random_state=42, verbose=-1)
CONFIGS = {  # (n_estimators, tau, lam)
    "A_tame": (300, 0.04, 1.0),
    "B_tau8": (300, 0.08, 1.0),
    "C_lam3": (300, 0.04, 0.3),
    "D_tiny": (100, 0.04, 1.0),
...
def make_objective(a: np.ndarray, w_eval: np.ndarray, a_bar: float, tau: float, lam: float):
    coef = a / (4.0 * a_bar)
    ...
    def obj(y_true, y_pred):
        ...
        g_ficr = -0.5 * coef * dR_dae * np.sign(e) * lam
        grad = np.clip(g_abs + g_ficr, -1.0, 1.0) * w_eval
...
def main() -> None:
    ...
    variants = ("base",) + tuple(f"{c}_{m}" for c in CONFIGS for m in ("solo", "blend"))
    ...
    for m0 in range(1, 12, 2):
        ...
        vmodels = {}
        for name, (n_est, tau, lam) in CONFIGS.items():
            vm = lgb.LGBMRegressor(objective=make_objective(a_arr, w_eval, a_bar_cf, tau, lam),
                                   n_estimators=n_est, **TAME)
            vm.fit(X[shared], y, init_score=init_tr)
            vmodels[name] = vm
        ...
        for tgt in TARGET_COLS:
            ...
            for name, vm in vmodels.items():
                g_val = np.clip((init_va + vm.predict(Xv[shared])) * cap, 0, cap)
                pairs += [(f"{name}_solo", g_val), (f"{name}_blend", 0.5 * g_opt + 0.5 * g_val)]
            for v, g in pairs:
